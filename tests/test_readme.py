...
def test_readme_generated(copie: Copie, base_answers: dict[str, str]) -> None:
    ...
    assert f'# {answers["template_name"]}' in content
    assert answers['template_description'] in content
...
def test_readme_title(copie: Copie, base_answers: dict[str, str]) -> None:
    """Test README title matches template_name."""
    answers = {**base_answers, 'copyright_license': 'MIT', 'template_name': 'copier-foo-bar'}
    result = copie.copy(extra_answers=answers)
    ...
    readme = result.project_dir / 'README.md'
    content = readme.read_text()
    assert content.startswith('# copier-foo-bar\n')
...
def test_readme_quickstart(copie: Copie, base_answers: dict[str, str]) -> None:
    """Test Quickstart section renders correctly."""
    answers = {**base_answers, 'copyright_license': 'MIT', 'vcs_github_path': 'user/copier-tool'}
    result = copie.copy(extra_answers=answers)
    ...
    assert '## Quickstart' in content
    assert 'pip install --upgrade copier copier-template-extensions' in content
    assert 'copier copy gh:user/copier-tool' in content
